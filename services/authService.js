...
const signToken = (user) =>
  jwt.sign(
    { id: user._id, email: user.email, verified: user.isEmailVerified, role: user.role },
...
const login = async ({ email, password }) => {
  ...
  const token = signToken({ ...user.toObject(), role });
  ...
  return {
    ...
    body: {
      success: true,
      token,
      user: { id: user._id, email: user.email, isEmailVerified: true, role },
    },
  };
};
...
const verifyEmail = async (rawToken) => {
  ...
  return {
    status: 200,
    body: { success: true, message: "Email verified successfully! You can now sign in." },
  };
};
...
const resendVerification = async ({ email }) => {
  ...
  if (!user) {
    return {
      status: 200,
      body: { success: true, emailSent: true, message: "If the email exists, a verification link has been sent." },
    };
  }
...
const forgotPassword = async ({ email }) => {
  ...
  if (!user) {
    return {
      status: 200,
      body: { success: true, emailSent: true, message: "If the email exists, a reset link has been sent." },
    };
  }
...
const resetPassword = async ({ token, password }) => {
  ...
  return {
    status: 200,
    body: { success: true, message: "Password reset successfully! You can now sign in." },
  };
};
